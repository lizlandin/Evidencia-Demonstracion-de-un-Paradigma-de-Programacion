...
<img width="700" height="image" alt="image" src="https://github.com/user-attachments/assets/8a995c67-9ece-40c0-817e-b487a23906c1" />
...
- Una de las principales diferencias entre ambas implementaciones es que en Racket se construyó una lista de adyacencia para almacenar los vecinos de cada nodo y así optimizar el código para que LeetCode aceptara la solución, en cambio, la implementación en Prolog trabajó directamente sobre la lista original de aristas proporcionada por el problema, lo cual hace la solución menos eficiente en comparación con la de Racket.
  - La razón por la que en Prolog no implementé una lista de adyacencia fue porque la programación lógica y su sintaxis me resultaban más complicadas de entender al momento de crear el código. Inicialmente hice una primera versión utilizando únicamente DFS pero al observar que el test case 3 de LeetCode no corría adecuadamente, pensé que el problema probablemente se debía a que la implementación no era lo suficientemente eficiente, por esta razón, consulté a una inteligencia artificial para que me explicara cómo podría adaptarse la solución que ya tenía pero  sumando una lista de adyacencia para así intentar optimizar el programa en Prolog. Creí que esta solución funcionaría porque algo similar ya me había ocurrido anteriormente en Racket, ya que la primera versión que desarrollé en ese lenguaje también utilizaba únicamente DFS, sin lista de adyacencia, pero cuando la ejecuté en LeetCode, no logró pasar todas las pruebas debido a su complejidad temporal, lo cual me obligó a implementar una lista de adyacencia en Racket, gracias a la cual el código logro ser aceptado. Por esta razón, asumí que implementar la misma optimización en Prolog permitiría que el caso de prueba 3 también funcionara correctamente, sin embargo, incluso después de implementar la versión con lista de adyacencia en Prolog, el caso de prueba más grande seguía sin producir un resultado adecuado y por esto opté por conservar la solución basada únicamente en DFS, ya que era la versión cuyo funcionamiento comprendía mejor y podía explicar con mayor claridad dentro de la evidencia.
  - A pesar de que durante el desarrollo del proyecto la sintaxis de Prolog me resultó más complicada de comprender que la de Racket, se puede observar que la implementación en el paradigma lógico terminó siendo considerablemente más corta, mientras que en Racket fue necesario utilizar varias funciones pequeñas para lograr resolver el problema, en Prolog gran parte de esta lógica se expresa mediante reglas, unificación y backtracking. Esto puede parecer algo irónico, pero demuestra que cada paradigma tiene fortalezas distintas y que ciertas soluciones pueden expresarse de forma más concisa en un lenguaje que en otro. Sin embargo, aunque la implementación en Prolog resultó más corta, la solución desarrollada en Racket fue la que mostró un mejor desempeño durante las pruebas, ya que logró ejecutar correctamente incluso el grafo más grande y complejo, el test case 3 de LeetCode, mientras que la versión de Prolog presentó dificultad para resolver ese caso y similares.
  - Durante clase se me sugirió que investigara por qué el grafo fallaba en Prolog, y que probablemente era porque los grafos de prueba eran muy sencillos y no tenían suficientes conexiones entre nodos. Sin embargo, al realizar varios casos de prueba de diferentes tipos de grafos con ayuda de inteligencia artificial, me di cuenta de que no era eso. Incluso cuando ingresaba grafos con cientos de conexiones y decenas de nodos, Prolog los procesaba rápidamente.
Después de experimentar con varios tipos de grafos, descubrí que el problema no es la cantidad de nodos ni de aristas por sí solas, sino una combinación muy específica de tres características que deben ocurrir al mismo tiempo:

1. El nodo de inicio tiene muchas conexiones (10 o más)
2. Los vecinos del nodo de inicio también tienen muchas conexiones
3. La solución pasa por el último vecino que Prolog explora
